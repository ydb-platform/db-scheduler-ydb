...
public class YdbTaskRepositoryTest {
    ...
    private GrpcTransport ydbTransport;
    private QueryClient ydbClient;
    private YdbTaskRepository repository;
    private YdbTaskEntityDao ydbDao;
    ...
    public void init() throws IOException {
        ydbTransport = GrpcTransport.forConnectionString(connectionString())
                .withAuthProvider(CloudAuthHelper.getAuthProviderFromEnviron())
                .build();
        ydbClient = QueryClient.newClient(ydbTransport).build();
        ydbDao = new YdbTaskEntityDao(ydbClient, YdbTaskRepository.DEFAULT_TABLE_NAME);
        ydbDao.createTable();
        ...
        repository = new YdbTaskRepository(taskResolver, schedulerName, serializer,
            YdbTaskRepository.DEFAULT_TABLE_NAME, ydbClient);
    }
    ...
    void testCreateIfNotExistsOk() {
        ...
        Collection<YdbTaskEntity> collection = ydbDao.readAll();

        assertThat(collection.isEmpty()).isFalse();

        YdbTaskEntity taskEntity = collection.iterator().next();
    ...
    @Test
    void testGetDueOk() {
        YdbTaskEntityBuilder builder = YdbTaskEntityBuilder.aTaskEntity().picked(false)
        ...
        List<YdbTaskEntity> entities = Arrays
    ...
    void testRemove() {
        ...
        YdbTaskEntityBuilder builder = YdbTaskEntityBuilder.aTaskEntity().taskName("task-1");
        ...
        List<YdbTaskEntity> tasks = StreamSupport
            .stream(Spliterators.spliteratorUnknownSize(
                ydbDao.readAll().iterator(),
    ...
    void testRescheduleWithDataOk() {
        ...
        YdbTaskEntity actual = ydbDao.readAll().iterator().next();
    ...
    void testGetDeadExecutions() {
        YdbTaskEntityBuilder shouldBePickedBuilder = YdbTaskEntityBuilder.aTaskEntity().picked(true)
            .taskName("task-1")
            .lastHeartbeat(Instant.now().minus(1, ChronoUnit.HOURS));

        YdbTaskEntityBuilder shouldNotBePickedBuilder = YdbTaskEntityBuilder.aTaskEntity().picked(true)
            .taskName("task-1")
    ...
    void testUpdateHeartbeat() {
        ...
        YdbTaskEntity task = YdbTaskEntityBuilder.aTaskEntity().taskName("task-1")
    ...
    void testGetExecutionsFailingLongerThan() {
        ...
        List<YdbTaskEntity> entities = Arrays.asList(taskBuilder.taskInstance("instance-1").build(),
    ...
    void testGetExecutionFound() {
        ...
        YdbTaskEntity entity = YdbTaskEntityBuilder.aTaskEntity().taskName("task-1")
    ...
    void testRemoveExecutions() {
        ...
        List<YdbTaskEntity> entities = Arrays.asList(builder.taskInstance("instance-1").build(),
        ...
        List<YdbTaskEntity> actual = ydbDao.readAll();
